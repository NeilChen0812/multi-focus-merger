...
def correct_perspective(image):
    ...
    rotated = cv2.warpAffine(
        image, M, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)  # 進行圖像旋轉
...
def main():
    folder_path = '03'  # 設置資料夾路徑，用來存放待拼接的圖像

    images = load_images_from_folder(
        folder_path, scale_factor=0.5)  # 從資料夾中讀取圖像並進行縮放
    ...
    if stitched_image is not None:
        ...
        corrected_image_resized = cv2.resize(
            corrected_image, (corrected_image.shape[1] // 2, corrected_image.shape[0] // 2))
