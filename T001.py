...
def main():
    ...
    if stitched_image is not None:
        ...
        final_image[start_y:start_y + images[0][1].shape[0], start_x:start_x + images[0][1].shape[1]] = images[0][1]
        
        # Display and save the first image
        print(f"File: {images[0][0]}, Image size: {images[0][1].shape}, Background size: {final_image.shape}")
        ...
        for i in range(1, len(images)):
            ...
            if offset_x < 0 or offset_x + images[i][1].shape[1] > width or start_y + images[i][1].shape[0] > height:
                print(f"Image {images[i][0]} cannot be placed due to size mismatch.")
                print(f"Image size: {images[i][1].shape}, Offset x: {offset_x}, Background width: {width}, Background height: {height}")
                continue
            ...
            final_image[start_y:start_y + images[i][1].shape[0], start_x:start_x + images[i][1].shape[1]] = images[i][1]
            ...
            print(f"File: {images[i][0]}, Image size: {images[i][1].shape}, Background size: {final_image.shape}")
            cv2.imshow(f'Aligned Image {i + 1}', final_image)
            cv2.imwrite(os.path.join(output_folder, f'{i + 1}.jpg'), final_image)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
            print(f"Processed image {i + 1} displayed and saved successfully.")
            
            # Print current background size
            print(f"Current background size after image {i + 1}: {final_image.shape}")
