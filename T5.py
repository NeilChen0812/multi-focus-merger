...
def create_panorama(images):
    ...
    for i in range(len(images) - 1):
        ...
        ptsA = np.float32([kp1[m.queryIdx].pt for m in good_matches[i]]).reshape(-1, 1, 2)
        ptsB = np.float32([kp2[m.trainIdx].pt for m in good_matches[i]]).reshape(-1, 1, 2)
        
        H, _ = cv2.findHomography(ptsB, ptsA, cv2.RANSAC, ransacReprojThreshold)
        cumulative_H = np.dot(cumulative_H, H)
        
        warped_img = cv2.warpPerspective(img2, cumulative_H, (img1.shape[1] + img2.shape[1], img2.shape[0]))
